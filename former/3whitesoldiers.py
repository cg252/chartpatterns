...
arr = yf.download("EURUSD=X", period="max", interval="1d", start="2005-01-01", actions="True")
...
for i in range(len(arr)-2):
    ...
    if (deltaTD > 0) and (deltaTMR > 0) and (deltaTMR > 1.4*deltaTD) and (abs((closeTMR-highTD)/closeTMR) < 0.0035) and (lowOVM > lowTMR) and (deltaOVM > (1.8*deltaTMR)) and (percentOVM > 0.4):
        ...
        try:
            Return5d.append(((closeList.iloc[i+5]-openList.iloc[i+3])/openList.iloc[i])*100)
            Return7d.append(((closeList.iloc[i+7]-openList.iloc[i+3])/openList.iloc[i])*100)
            Return10d.append(((closeList.iloc[i+10]-openList.iloc[i+3])/openList.iloc[i])*100)
        except:
...
print(str(len(Return5d)) + ' Cases found')
