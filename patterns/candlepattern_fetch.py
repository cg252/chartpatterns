...
arr = yf.download(TICKER, start="2022-09-29", interval="1h")
...
candlearray = pd.DataFrame()
candlelist = []
for candle in candle_names:
    candlearray[candle] = getattr(talib, candle)(opens, highs, lows, closes)


from itertools import compress

candlearray['candlestick_pattern'] = np.nan
candlearray['candlestick_match_count'] = np.nan

for index, row in candlearray.iterrows():

    # no pattern found
    if len(row[candle_names]) - sum(row[candle_names] == 0) == 0:
        candlearray.loc[index,'candlestick_pattern'] = "NO_PATTERN"
        candlearray.loc[index, 'candlestick_match_count'] = 0
    # single pattern found
    else:
        # bull pattern 100 or 200
        if any(row[candle_names].values > 0):
            pattern = list(compress(row[candle_names].keys(), row[candle_names].values != 0))[0] + '_Bull'
            candlearray.loc[index, 'candlestick_pattern'] = pattern
            candlearray.loc[index, 'candlestick_match_count'] = 1
...
data = {"Close": closes,
        ...
        "adx": ADXList,
        "macd": MACDList,
        "candle": candlearray['candlestick_pattern'],
